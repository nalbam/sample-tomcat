...
podTemplate(label: label, containers: [
  ...
  node(label) {
    ...
    if (SOURCE_LANG == "java") {
      stage("Build") {
        container("maven") {
          ...
          """
          notify("good", "Build Success", "Build Success: $IMAGE_NAME-$VERSION <$PIPELINE|#$BUILD_NUMBER>")
        }
      }
    }
    ...
    if (BRANCH_NAME == "master") {
      ...
      stage("Proceed") {
        container("builder") {
          notify("#439FE0", "Deply Confirm", "Proceed Production?: $IMAGE_NAME-$VERSION <$PIPELINE|#$BUILD_NUMBER>")
...
def notify(COLOR, TITLE, MESSAGE) {
  try {
    if (SLACK_TOKEN) {
      sh "curl -sL toast.sh/helper/slack.sh | bash -s -- --token=$SLACK_TOKEN --color=$COLOR --title='$TITLE' '$MESSAGE'"
    }
  } catch (ignored) {
  }
}
