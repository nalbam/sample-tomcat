  ...
  node(label) {
    ...
    if (SOURCE_LANG == "java") {
      stage("Build") {
        container("maven") {
          ...
          """
          TITLE="Build Success"
          TEXT="$IMAGE_NAME-$VERSION"
          LINK="jenkins blue <$PIPELINE|#$BUILD_NUMBER>"
          notify("good", "$TITLE", "$TEXT", "$LINK")
        }
      }
    }
    ...
    if (BRANCH_NAME == "master") {
      ...
      stage("Proceed") {
        container("builder") {
          TITLE="Deply Confirm"
          TEXT="$IMAGE_NAME-$VERSION to Production"
          LINK="jenkins blue <$PIPELINE|#$BUILD_NUMBER>"
          notify("#439FE0", "$TITLE", "$TEXT", "$LINK")
...
def notify(COLOR, TITLE, TEXT, LINK) {
  try {
    if (SLACK_TOKEN) {
      sh """
        curl -sL toast.sh/helper/slack.sh | bash -s -- \
                 --token='$SLACK_TOKEN' --color='$COLOR' \
                 --title='$TITLE' --footer='$LINK' '$TEXT'
      """
    }
  } catch (ignored) {
  }
